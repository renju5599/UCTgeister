...
NodeNum UCTandAlphaBeta::Choice()
{
	...
	for (NodeNum nextnode : Nodes[playnodenum].nextnodes)
	{
		...
		if (chmax(maxvalue, (double)Nodes[nextnode].value.win / Nodes[nextnode].value.play + exp(Nodes[nextnode].value.NN_score) / softmax * 0.01))
	...
	for (NodeNum nextnode : Nodes[playnodenum].nextnodes)
	{
		...
		printf("\tComp:%.6f", (Nodes[nextnode].value.play == 0 ? 0 : (double)Nodes[nextnode].value.win / Nodes[nextnode].value.play + exp(Nodes[nextnode].value.NN_score) / softmax * 0.01));

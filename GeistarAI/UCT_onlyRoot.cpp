...
void UCT::SetNode(Recieve str)
{
	...
	int ennum = __popcnt64(Nodes[1].board.enemy);	//生きている敵の数
	int sum = 0;
	for (int i = 8; i < 16; i++)
	{
		sum += Red::eval[i];
		...
		for (int i = 8; i < 16; i++)
		{
			...
			if (border > r)
			{
				...
				else
				{
					...
					assert(onPiece(Nodes[1].board.enemy, Nodes[1].board.pieces[i]));
				}
				sum -= Red::eval[i];
				colored[i] = 1;
				break;
	...
	Nodes[1].board.enblue = Nodes[1].board.enemy & ~Nodes[1].board.enred;
	for (int i = 0; i < 8; i++)
	{
...
void UCT::search_tree(int& turnnum, int& search_node, bool& nowPlayer)
{
	while (turnnum < MAXPLAY && !Nodes[search_node].nextnodes.empty())	//木の端まで見る
	{
		// 勝負がついていたら終わり
		if (
			__popcnt64(Nodes[search_node].board.enemy) < 2 ||
...
NodeNum UCT::Choice()
{
	...
	for (NodeNum nextnode : Nodes[playnodenum].nextnodes)
	{
		...
		if (chmax(maxvalue, (double)Nodes[nextnode].value.win / 2.0 / Nodes[nextnode].value.play + exp(Nodes[nextnode].value.NN_score) / softmax * 0.01))
		{
			choice_nodenum = nextnode;
		}
	}
